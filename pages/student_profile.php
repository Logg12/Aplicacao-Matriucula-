...
/////////////////// alteracao dados
$buscaDataAluno = "SELECT * FROM aluno WHERE nome= '$nomeUsuarioAtual'";
$buscaDataUsuario = "SELECT * FROM usuario WHERE nome= '$nomeUsuarioAtual'";
...
if (isset($_POST['emailChange']) || isset($_POST['cpfChange']) || isset($_POST['sobrenomeChange']) || isset($_POST['nomeChange']) || isset($_POST['enderecoChange'])) {
    ...
    $atualizarUsuario = "UPDATE usuario SET email = '$emailChange'
                        , nome = '$nomeChange $sobrenomeChange'  WHERE nome = '$nomeUsuarioAtual'";
    $atualizarAluno = "UPDATE aluno SET nome = '$nomeChange $sobrenomeChange' 
                    , cpf = '$cpfChange' 
                    , endereco = '$enderecoChange'
                        WHERE nome = '$nomeUsuarioAtual'";
    ...
    if ($queryAtualizacaoA && $queryAtualizacaoU) {
        ...
        $_SESSION['dados']['userLogado'] = "$nomeChange" . " $sobrenomeChange";
        $_SESSION['dados']['userEmail'] = "$emailChange";
...
if (isset($_POST['passNew']) && isset($_POST['passOld']) && isset($_POST['passOld2'])) {
    ...
    if ($senhaAntiga === $senhaAntiga2) {
        if ($senhaAntiga === $senhaAtual) {
            ...
            $alterarSenha = "UPDATE usuario SET password = '$senhaNova' WHERE nome = '$nomeUsuarioAtual'";
            ...
            if (mysqli_query($connectionDb, $alterarSenha)) {
                ...
                $_SESSION['dados']['senha'] = $senhaNova;
            } else {
...
if (isset($_POST['tels'])) {
    ...
    $tels = $_POST['tels'];
    $zap = isset($_POST['zapCheck']) ? 1 : 0;

    $deleteTels = "DELETE FROM telefone WHERE aluno_id = $id";
    $resultadoDel = mysqli_query($connectionDb, $deleteTels);

    if ($resultadoDel) {
        foreach ($tels as $telefone) {

            $insertAllTels = "INSERT INTO telefone (aluno_id, numero, whatsapp) VALUES ($id, '$telefone', $zap)";
            mysqli_query($connectionDb, $insertAllTels);
        }

        $status3 = "<span class='text-success'>Modificações Atualizadas!</span>";
        echo "<style>
        .status3 { 
            display: inline !important; 
        }
        ...
        $status3 = "Erro na Modificação de Dados: " . $connectionDb->error;
        echo "<style>
        .status3 {
            color: var(--bs-danger);
            display: inline !important; 
...
<body class="g-sidenav-show bg-gray-100" id="bd">
                    ...
                    <div class="col-auto my-auto">
                        <div class="h-100">
                            <h5 class="mb-1">
                                ...
                                echo ucwords($_SESSION['dados']['userLogado']) ?>
                            </h5>
                            <p class="mb-0 font-weight-bold text-sm">
                                Tipo de perfil :
                                <?php echo ucwords($_SESSION['dados']['tipoUser']) ?>
                            </p>
                        </div>
                    </div>
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input class="form-control" name="nomeChange" type="text"
                                                            value="<?php echo $nome; ?>" id="nomeInput" required>
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                    <div class="form-group">
    ...
    //     var checkbox = document.getElementById("checkboxZap");
    ...
    function criarTelArea() {
        ...
        colDiv2.className = 'col-sm-1 d-flex justify-content-center align-items-center';

        var divZap = document.createElement('div');
        divZap.className = 'form-check form-check-inline pt-3';

        // Create the input element with class 'form-check-input' and type 'checkbox'
        var inputZap = document.createElement('input');
        inputZap.className = 'form-check-input';
        inputZap.type = 'checkbox';
        inputZap.id = 'checkboxZap';
        inputZap.name = 'zapCheck'
        inputZap.setAttribute('onchange', 'atualizarValor()');

        // Create the label element with class 'form-check-label'
        var labelZap = document.createElement('label');
        labelZap.className = 'form-check-label';

        // Create a bold element
        const boldZap = document.createElement('b');
        boldZap.textContent = 'Whatsapp';

        // Append the bold element to the label element
        labelZap.appendChild(boldZap);

        // Append the input and label elements to the div element
        divZap.appendChild(inputZap);
        divZap.appendChild(labelZap);


        colDiv2.appendChild(divZap);
